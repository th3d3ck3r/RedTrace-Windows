...
public sealed class TerminalView : RichEditBox, IDisposable
{
    ...
    public TerminalView(Action<byte[]> send)
    {
        ...
        Background = new SolidColorBrush(Windows.UI.Colors.Transparent); BorderThickness = new Thickness(0);
    ...
    void Flush()
    {
        ...
        Document.Selection.SetRange(int.MaxValue, int.MaxValue); ScrollIntoView(ScrollIntoViewAlignment.Default);
    }
    void OnCharacter(UIElement sender, CharacterReceivedRoutedEventArgs e) { if (e.KeyCode >= 32) { Send(Encoding.UTF8.GetBytes(char.ConvertFromUtf32((int)e.KeyCode))); e.Handled = true; } }
